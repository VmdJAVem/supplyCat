...
void generateRookMoves(moveLists * ml, color c, Tablero * t) {
	...
	while (rooks) {
		...
		for (int d = 0; d < 4; d++) {
			...
			if (blockers) {
				int firstBlocker;
				if (d < 2) // north/east (positive direction)
					firstBlocker = __builtin_ctzll(blockers);
				else // south/west (negative direction)
					firstBlocker = 63 - __builtin_clzll(blockers);

				// Generate moves up to but not including the first blocker
				bitboard before;
				if (d < 2)
				...
				// Add capture if first blocker is enemy
				...
				// No blockers - all squares are valid quiet moves
				while (ray) {
...
void generateBishopMoves(moveLists * ml, color c, Tablero * t) {
	...
	while (bishops) {
		...
		for (int d = 0; d < 4; d++) {
			...
			if (blockers) {
				int firstBlocker;
				if (d > 1) // down-left/down-right (positive direction)
					firstBlocker = __builtin_ctzll(blockers);
				else // up-left/up-right (negative direction)
					firstBlocker = 63 - __builtin_clzll(blockers);
